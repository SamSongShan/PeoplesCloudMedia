...
public class DividerGridItem extends RecyclerView.ItemDecoration {
     ...
     * 使用宽分隔线 ATTRS_B 时使用
     */
    public DividerGridItem(Context context, int[] divider) {
     ...
     * 头部不添加分隔线时调用 -- 默认全部添加
     */
    public DividerGridItem(Context context, int headerCount) {
        this(context);
     ...
     */
    public DividerGridItem(Context context, boolean isDrawAllLine) {
        this(context);
        this.isDrawAllLine = isDrawAllLine;
    }

    public DividerGridItem(Context context, int headerCount, int[] divider) {
